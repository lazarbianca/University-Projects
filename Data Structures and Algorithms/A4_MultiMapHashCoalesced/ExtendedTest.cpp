...
void testIteratorSteps(MultiMap& mm) {
	cout << "	testing iterator steps...\n";
	/*int ok = 0;
	for (int l = 0; l < mm.mapCapacity && ok; l++)
	{
		cout << l << " " << "key: " << mm.elements[l].first << " value: " << mm.elements[l].second << " -- next: " << mm.next[l] << endl;
	}*/
	int count = 0;
	...
	while (mmit.valid()) {
		/*if (ok)
		{
			cout << mmit.getCurrent().first << "  " << mmit.getCurrent().second << endl;
...
void testRemove() {
    ...
    vector<TValue> v;
	v=m.search(1);
	//cout << "v size= " << v.size() << "   v.at(0) = " << v.at(0) << endl;
...
void testMaxKey()
{
	...
	m.remove(31, 101);
	assert(m.maxKey() == 25);
	cout << "Finished extra operation maxKey() tests!\n";
}
