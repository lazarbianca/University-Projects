...
from topological import topologicalSortPredecessorCounting, highestCostPath, printPathPredecessors
...
def printMenu():
    ...
    print("20. Check if the graph is a DAG and perform a topological sorting 0(predecessor counters)")
# if it is a DAG, find one highest cost path between two given vertices (O(m+n))
...
if __name__ == '__main__':
    ...
    while continueRun is True:
        ...
        if option == 0:
            ...
            # try:
            #     l = g.topological_sorting_based_on_predecessor_counting()
            #     print("Topological sorting:", l)
            #     print("The given graph is a DAG")
            # except ValueError:
            #     print("The graph has cycles, so we cannot perform a topological sorting. Therefore, the given graph is not a DAG")
            sortedList = topologicalSortPredecessorCounting(graph)
            if sortedList is None:
                print("The graph has cycles => no topological sorting possible; graph is NOT a Directed Acyclic Graph ")
            else:
                print("graph is a Directed Acyclic Graph")
                print("Topological sorting:", sortedList)
                u = int(input("Enter start vertex: "))
                v = int(input("Enter end vertex: "))
                previousVertices, maxCostPath = highestCostPath(graph, sortedList, u, v)
                print("Highest Cost Path:", maxCostPath)
                printPathPredecessors(v, u, v, previousVertices)
