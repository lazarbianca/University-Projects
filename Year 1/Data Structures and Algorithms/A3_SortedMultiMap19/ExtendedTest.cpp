...
void testIteratorSteps(SortedMultiMap& m) {
	...
	for (int i = 0; i < count / 2; i++) {
		//cout << "current elem: " << smmi.getCurrent().first << ", " << smmi.getCurrent().second << endl << endl;
		smmi.next();	// crapa (2) on second next() - not valid iterator => it calls the exception, which goes unhandled
	}
	smmi.first();
	int c = 0;
...
void testSearch(Relation r) {
	...
	for (int i = kMin; i <= kMax; i++) {
        vector<TValue> v= smm.search(i);
		//cout << "-> " << v.size() << endl;
        assert(v.size()==2);	//aici crapa (1): " A LIST_ENTRY has been corrupted (i.e. double remove)."
	}
...
void testIterator(Relation r) {
	...
	int n = keys.size();
	for (int i = 0; i < n; i++) {
	    //cout << "adding " << keys[i] << " with value: " << 100 << endl;
	    smm.add(keys[i], 100);
	    if (keys[i]%2==0)	{
		    //cout << "adding " << keys[i] << "with value: " << 200 << endl;
		    smm.add(keys[i], 200);
	    }
	}

	//cout << "seconds before disaster...\n";
	testIteratorSteps(smm);
...
void testReplace() {
	...
	smm.replace(10, 100, 150);
	smm.replace(10, 200, 150);
	SMMIterator it = smm.iterator();
	it.first();
	while (it.valid())
	{
		//cout << it.getCurrent().first << " with value: " << it.getCurrent().second << endl;
		if (it.getCurrent().first == 10)
		{
			assert(it.getCurrent().second == 150);
		}
		else
		{
			assert(it.getCurrent().second != 150);
	...
	cout << "Test replace (finished)\n";
}

void testAllExtended() {
	testCreate();    //passes
	testIterator();  //passes without testIteratorSteps
	testSearch();
	testRemove();
	testRelations();
	testReplace();
